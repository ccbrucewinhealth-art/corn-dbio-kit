fn export_single_page(
    ...
    if let Some(mut cursor) = conn.execute(&sql, ())? {
        let col_count_i16 = cursor.num_result_cols()?;
        if col_count_i16 <= 0 {
            if !silent {
                print_export_row(
                    "PAGE",
        ...
        let col_count = col_count_i16 as usize;
        let mut quoted_columns = Vec::<String>::with_capacity(col_count);
        for col_index in 1..=col_count {
            let col_name = cursor.col_name(col_index as u16)?;
            quoted_columns.push(quote_ident(cli.db_type, &col_name));
            // 保留原始欄位名稱供 hide-primary-key 比對
            // col_name 對不同 driver 可能有大小寫差異，稍後統一 lowercase 比對
        }
        ...
        while let Some(batch) = row_set_cursor.fetch()? {
            for row_index in 0..batch.num_rows() {
                ...
                for col_index in 0..batch.num_cols() {
                    ...
                    let value = to_insert_value(
                        batch
                            .at_as_str(col_index, row_index)
                            .map_err(|e| {
                                anyhow!(
                                    "utf8 decode failed at row {row_index}, col {col_index}: {e}"
                                )
                            })?,
                    );
                    if value == "NULL" {
...
fn export_table(conn: &odbc_api::Connection<'_>, cli: &Cli, silent: bool, table: &str) -> Result<usize> {
