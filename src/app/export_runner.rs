fn execute_export_with_cancel(
    ...
    let env = match Environment::new() {
        Ok(v) => v,
        Err(e) => {
            return Err(anyhow!("cannot create ODBC environment: {e}"));
        }
    };

    let conn_str = build_connection_string(&cli);
    let conn = match env.connect_with_connection_string(&conn_str, ConnectionOptions::default()) {
        Ok(v) => v,
        Err(e) => {
    ...
    let all_objects = match query_first_column_as_vec(&conn, object_sql) {
        Ok(v) => v,
        Err(e) => {
    ...
    if let ObjectSelector::SqlFile(sql_file) = &object_selector {
        ...
        match export_sql_query_result(&conn, &cli, sql_file) {
            Ok(rows) => {
                if !silent {
                    print_export_row(
                        "OBJECT",
    ...
    for (table_idx, table) in selected_objects.into_iter().enumerate() {
        ...
        match export_table(&conn, &cli, silent, &table) {
            Ok(rows) => {
                if !silent {
                    print_export_row(
                        "TABLE",
