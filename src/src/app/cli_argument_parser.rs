...
fn parse_import_content_format(raw: &str) -> Result<ImportContentFormat> {
    match raw.trim().to_ascii_lowercase().as_str() {
        "sql" => Ok(ImportContentFormat::Sql),
        "csv" => Ok(ImportContentFormat::Csv),
        "md" | "markdown" => Ok(ImportContentFormat::Md),
        "excel" | "xls" | "xlsx" => Ok(ImportContentFormat::Excel),
        "sql-lite" | "sqlite" => Ok(ImportContentFormat::SqlLite),
        _ => Err(anyhow!(
            "unsupported import-content-format: {raw} (allowed: sql, csv, md, excel, sqlite)"
        )),
    }
}
...
fn parse_flow_mode(raw: &str) -> Result<FlowMode> {
    match raw.trim().to_ascii_lowercase().as_str() {
        "export-data" | "export" => Ok(FlowMode::Export),
        "import-data" | "import" => Ok(FlowMode::Import),
        "export-schema" => Ok(FlowMode::ExportSchema),
        "import-schema" => Ok(FlowMode::ImportSchema),
        _ => Err(anyhow!(
            "unsupported flow: {raw} (allowed: export-data, import-data, export-schema, import-schema; legacy: export, import)"
        )),
    }
}
...
fn default_tui_cli() -> Cli {
    Cli {
        ...
        import_collation: String::new(),
        import_content_format: ImportContentFormat::Md,
        import_content_src: PathBuf::from("./export/export.sqlite"),
    }
}
...
fn apply_env_to_cli(cli: &mut Cli) -> Result<()> {
    ...
    if let Some(v) = env_non_empty("DB2SQL_EXPORT_CONTENT_FORMAT")
        .or_else(|| env_non_empty("DB2SQL_EXPORT_MEDIA_FORMAT"))
        .or_else(|| env_non_empty("DB2SQL_EXPORT_OUTPUT_FORMAT"))
        .or_else(|| env_non_empty("DB2SQL_OUTPUT_FORMAT"))
    {
    ...
    if let Some(v) = env_non_empty("DB2SQL_EXPORT_COLLATION") {
        cli.export_collation = v;
    }
    ...
    if let Some(v) = env_non_empty("DB2SQL_IMPORT_COLLATION") {
        cli.import_collation = v;
    }
    if let Some(v) = env_non_empty("DB2SQL_IMPORT_CONTENT_FORMAT") {
        cli.import_content_format = parse_import_content_format(&v)?;
    }
    if let Some(v) = env_non_empty("DB2SQL_IMPORT_CONTENT_SRC") {
        cli.import_content_src = PathBuf::from(v);
    }
    Ok(())
}
...
fn print_help(program: &str) {
    ...
    println!();
    println!("Arguments:");
    println!("  --flow              流程：export-data|import-data|export-schema|import-schema，預設 export-data");
    ...
    println!("  --export-content-format    輸出內容格式：sql|sql-lite|csv|excel|json|xml|md，預設 sql");
    ...
    println!("  --export-collation         SQL Server 匯出連線/查詢用 collation（可選）");
    println!("  --import-host       匯入目標 host（flow=import-data 時使用）");
    println!("  --import-port       匯入目標 port（flow=import-data 時使用）");
    println!("  --import-db-type    匯入目標資料庫類型：mssql|oracle|mysql|postgresql|sql-lite");
    println!("  --import-user       匯入目標使用者（flow=import-data 時使用）");
    println!("  --import-password   匯入目標密碼（flow=import-data 時使用）");
    println!("  --import-db-name    匯入目標資料庫（flow=import-data 時使用）");
    println!("  --import-collation  SQL Server 匯入連線/查詢用 collation（可選）");
    println!("  --import-content-format 匯入內容格式：sql|csv|md|excel|sqlite，預設 md");
    println!("  --import-content-src    匯入檔案或匯出壓縮檔來源");
    println!();
    ...
    println!("  /show                    顯示目前設定");
    println!("  /run-export-data         執行匯出");
    println!("  /run-import-data         執行匯入");
    println!("  /help                    顯示指令說明");
    println!("  /quit                    離開 TUI");
    println!();
    println!("Example:");
    println!(
        "  {program} --mode cli --flow export-data --export-db-type mssql --export-host 192.168.0.25 --export-port 1433 --export-user TRC201 --export-password 'syscom#1' --export-db-name TRC_RData --export-objects '^TRM_.*' --export-condition \"SnId>0\" --export-order-by \"SnId desc\" --export-hide-primary-key true --export-content-format sql --export-compress zip --export-delete-after-compress false --export-page-size 10000"
    );
}
...
fn parse_args() -> Result<ParseOutcome> {
    ...
    let mut export_collation = base_cli.export_collation;
    ...
    let mut import_collation = base_cli.import_collation;
    let mut import_content_format = base_cli.import_content_format;
    let mut import_content_src = base_cli.import_content_src;
    ...
    while i < args.len() {
        ...
        match key.as_str() {
            ...
            "--export-content-format" | "--export-media-format" | "--export-output-format" | "--output-format" => {
            ...
            "--export-collation" => {
                export_collation = need_value(i, key, &args)?;
                i += 2;
            }
            "--import-host" => {
            ...
            "--import-collation" => {
                import_collation = need_value(i, key, &args)?;
                i += 2;
            }
            "--import-content-format" => {
                import_content_format = parse_import_content_format(&need_value(i, key, &args)?)?;
                i += 2;
            }
            "--import-content-src" => {
                import_content_src = PathBuf::from(need_value(i, key, &args)?);
                i += 2;
            }
            _ => {
