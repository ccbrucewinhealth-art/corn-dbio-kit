fn cli_snapshot_lines(cli: &Cli) -> Vec<String> {
    vec![
        ...
        format!("export-content-format={:?}", cli.output_format),
        format!(
            "export-compress={}",
            cli.compress
                .map(|v| format!("{:?}", v))
        ...
        format!("export-collation={}", cli.export_collation),
        ...
        format!("import-collation={}", cli.import_collation),
        format!(
            "import-content-format={:?}",
            cli.import_content_format
        ),
        format!("import-content-src={}", cli.import_content_src.display()),
    ]
}
