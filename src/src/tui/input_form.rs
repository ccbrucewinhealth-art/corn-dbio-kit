fn apply_set_field(cli: &mut Cli, field: &str, value: &str) -> Result<()> {
    match field {
        ...
        "export-content-format" | "export-media-format" | "export-output-format" => {
            cli.output_format = parse_output_format(value)?
        }
        ...
        "export-collation" => cli.export_collation = value.to_string(),
        ...
        "import-collation" => cli.import_collation = value.to_string(),
        "import-content-format" => cli.import_content_format = parse_import_content_format(value)?,
        "import-content-src" => cli.import_content_src = PathBuf::from(value),
        _ => {
            return Err(anyhow!(
                "unsupported field: {field} (allowed: flow,db-type,host,port,user,password,db-name,objects,condition,order-by,hide-primary-key,export-content-format,compress,delete-after-compress,output-directory,recsize,page-size,page-no,export-collation,import-host,import-port,import-db-type,import-user,import-password,import-db-name,import-collation,import-content-format,import-content-src)"
            ))
        }
    }
    Ok(())
...
fn field_options(field: &str) -> Option<Vec<&'static str>> {
    match field {
        "flow" => Some(vec!["export-data", "import-data", "export-schema", "import-schema"]),
        ...
        "export-content-format" => Some(vec!["sql", "sql-lite", "csv", "excel", "json", "xml", "md"]),
        ...
        "import-content-format" => Some(vec!["sql", "csv", "md", "excel", "sqlite"]),
        _ => None,
    }
}
...
fn get_field_value(cli: &Cli, field: &str) -> String {
    match field {
        ...
        "export-content-format" => output_format_to_str(cli.output_format).to_string(),
        "export-compress" => cli
            .compress
        ...
        "export-collation" => cli.export_collation.clone(),
        ...
        "import-collation" => cli.import_collation.clone(),
        "import-content-format" => import_content_format_to_str(cli.import_content_format).to_string(),
        "import-content-src" => cli.import_content_src.display().to_string(),
        _ => String::new(),
    }
}
