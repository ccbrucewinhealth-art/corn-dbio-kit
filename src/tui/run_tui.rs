pub(crate) fn run_tui(mut cli: Cli) -> Result<Cli> {
    ...
    let tr = |lang: Lang, key: &str| -> String {
        match (lang, key) {
            ...
            (Lang::Tw, "commands_short") => "指令: /set /unset /show /show-hot-key /run-export /quit".to_string(),
            (Lang::En, "commands_short") => "commands: /set /unset /show /show-hot-key /run-export /quit".to_string(),
            (Lang::Ja, "commands_short") => "コマンド: /set /unset /show /show-hot-key /run-export /quit".to_string(),
            ...
            (Lang::Tw, "help_line") => "說明: /set <field> <value>, /unset <field>, /show, /run-export, /quit".to_string(),
            (Lang::En, "help_line") => "help: /set <field> <value>, /unset <field>, /show, /run-export, /quit".to_string(),
            (Lang::Ja, "help_line") => "ヘルプ: /set <field> <value>, /unset <field>, /show, /run-export, /quit".to_string(),
            ...
            (Lang::Tw, "status_default") => "/run-export 進行執行".to_string(),
            (Lang::En, "status_default") => "/run-export to execute".to_string(),
            (Lang::Ja, "status_default") => "/run-export で実行".to_string(),
            ...
            terminal.draw(|f| {
                ...
                if help_popup {
                    ...
                    let help_lines = vec![
                        ...
                        Line::from("----------------------------------------------"),
                        Line::from("選單"),
                        Line::from("1) 匯出   : 設定 flow=export 並執行"),
                        Line::from("2) 匯入   : 設定 flow=import 並執行"),
            ...
            match event::read()? {
                ...
                match key.code {
                            ...
                            if let Some((_, cmd)) = menu_items.get(menu_selected_idx) {
                                match *cmd {
                                    "__menu_export__" => {
                                        ...
                                        input = "/run-export".to_string();
                                        focus_area = FocusArea::Input;
                                        current_page = ScreenPage::Main;
                                        logs.push("menu: 匯出 (flow=export)".to_string());
                                    ...
                                    "__menu_import__" => {
                                        ...
                                        focus_area = FocusArea::Input;
                                        current_page = ScreenPage::Main;
                                        logs.push("menu: 匯入 (flow=import)".to_string());
                                        logs.push("請按下 Enter 確認執行".to_string());
                                    }
                                    "__menu_db_ops__" => {
                                        current_page = ScreenPage::DbOps;
                    ...
                    KeyCode::Enter => {
                        if table_edit_popup {
                            ...
                            if table_edit_focus == TableEditFocus::InsertBtn {
                                if !table_edit_table_name.is_empty() && !table_preview_cols.is_empty() {
                                    let vals = table_preview_cols.iter().map(|_| String::new()).collect::<Vec<_>>();
                                    match insert_table_row(&cli, &table_edit_table_name, &table_preview_cols, &vals) {
                                        Ok(sql) => {
                                            table_edit_sql = sql;
                                            logs.push("table row inserted".to_string());
                        ...
                        } else if cmd == "/run-export" {
                            logs.push(match cli.flow {
                                FlowMode::Export => tr(cli.lang, "run_start"),
                                FlowMode::Import => tr(cli.lang, "run_start_import"),
                            });
